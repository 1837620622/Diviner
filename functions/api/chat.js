...
// 把联网参详资料逐条拼成注入文本（标题、摘要、链接 + 校正指令）。
function buildSourcesBlock(sources) {
  const lines = sources.map(
    (s, i) => `${i + 1}. 标题：${s.title}\n   摘要：${s.snippet}\n   链接：${s.url}`
  );
  // 块标题【联网参详资料】须与前端 SYSTEM_PROMPT 中监听的名称逐字一致，
  // 全站界面用语亦统一为「联网参详」，否则提示词中该条指令永不触发。
  return [
    '【联网参详资料】',
    ...lines,
    '',
    '以上为联网参详所得。作答时须以你自身的易学知识与上述资料相互校正；若两者冲突，须在『象数解析』中明言冲突所在，不可含糊。',
